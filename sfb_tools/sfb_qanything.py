...
class YoudaoQAnything:
    ...
    def chat(self, q):
        ...
        if 'result' in response_data and 'response' in response_data['result']:
            ...
            # 替换字符串内部的三个点
            code_module_cleaned = code_module_cleaned.replace('```', '')
            
            # 将code_module_cleaned写入日志
...
if __name__ == '__main__':
    # 初始化 YoudaoQAnything 对象
    youdao_qanything = YoudaoQAnything()
    # 设置日志记录
    youdao_qanything.setup_logging()

    # 查询知识库列表
    # youdao_qanything.kbList()

    # 查询文档列表
    # youdao_qanything.fileList()
